...
my_image_path2 = os.getenv('HOME')+'/Downloads/2323.png'
...
##interpolation (두번째 사진)
#width2 = 900
#height2 = 900
#img_stack_sm5 = np.zeros((width1, height1, 1040),dtype = int)
#img_sm9 = cv2.resize(img_sticker, (height2, width2), interpolation=cv2.INTER_CUBIC)#보간법. cubic 이 linear 보다 좋음.
...
sticker_path = os.getenv('HOME')+'/Downloads/2323.png'
...
cv2.imwrite("imgsm11.png",img_sm10)
...
#cv2.imshow('e1', e1)
#cv2.imwrite("addweighted5.png",e1)

    
list_landmarks=[]
#mlib_rects = [[1,1,1],[2,2,2],[10,10,10],[20,20,30],[30,30,33],[50,50,50],[60,60,60],[70,70,70],[80,80,80],[90,90,90]]
#for mlib_rect in mlib_rects:

    
#landmark predictor 선언!!!!!!!!!!!!!!!!!!!!!

points = landmark_predictor(img_sticker,1)
#list_points = list(map(lambda p: (p.x,p.y), points.parts()))
#list_landmarks.append(list_points)
print(points)
...
cv2.imwrite("imgsm111.png",img_show_rgb1)

roi = img_sm10[vpos:vpos + 200, hpos: hpos + 250]

img2gray = cv2.cvtColor(img_sm10, cv2.COLOR_BGR2GRAY) 
